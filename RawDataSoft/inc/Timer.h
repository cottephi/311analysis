...
{
 private:
  timeval startTime;// sys/time.h
  timeval lastTime;// sys/time.h
 ...
 public:
  ...
  void start()
  {
    gettimeofday(&startTime, NULL);//c++ func
    lastTime = startTime;
  }
  ...
  {
    timeval curTime;
    gettimeofday(&curTime, NULL);//c++ func
    
    double tdiff = duration(lastTime, curTime);//c++ func
  ...
  double stop(bool _echo = true)
  {
    ...
    double tdiff = duration(startTime, curTime);//c++ func
