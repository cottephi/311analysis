...
std::string HuffDataCompressor::GetCodeFromValue(short val)
{
  ...
  if(std::abs(val) > m_MaxDiff && val != m_NSeqRepVal)
    {
      cout << __FILE__<<", "<<__LINE__<<" the value '"<<val<<"' exceeds range" << endl;
      return emptystr;
    }
  ...
  //
  cout << __FILE__<<", "<<__LINE__<<" the value '"<<val<<"' could not be found" << endl;
  return emptystr;
}
...
std::string HuffDataCompressor::GetBinaryString(short val, size_t strsize)
{
  ...
  cout << __FILE__<<", "<<__LINE__<<" the string looks too long" << endl;
					 ...
					 std::vector<BYTE> &bin_out )
{
  if(!SetNbitsAdc( nbadc ))
    {
      bin_out.clear();
      cout << "ADC "<<nbadc<<" bits exceeds max supported "<<m_MaxAdcBits << endl;
      return;
    }
					    ...
					    std::vector<BYTE> &bin_out )
{
  ...
  if( seqlen * nch != raw_in.size() )
    {
      cout << "Length of raw data vector does not match with expected " << endl;
      return;
    }
					    ...
					    std::vector<BYTE> &bin_out )
{
  ...
  if( nch != raw_in.size() )
    {
      cout << "Number of ch in raw data vector does not match with expected " << endl;
      return;
    }
  ...
  for(size_t i=0;i<nch;i++)
	...
	{
	  cout << "No support for compression of unequal sequence lenghts at the moment" << endl;
	  bin_out.clear();
	  return;
	}
					      ...
					      std::vector<adc16_t> &adc )
{
  ...
  while( adc.size() != nch*seqlen )
	...
	{
	  cout << "There seems to be a problem with decoding" << endl
      ...
      if(!iscomp) // uncompressed
	{
	  ...
	  if( bitqueue.size() < m_NbitsHC ) 
	    {
	      cout << "Fatal decoding error has been encountered : " << endl
      ...
      if(chdata.size() == seqlen)
	{
	  ...
	  if(!bitqueue.empty())
		...
		{
		  cout << "Fatal decoding error had been encounter : " << endl
      ...
      if(chread == nch)
	{
	  ...
	  if(bitqueue.size() > 0) // should be no more than 8
	    {
	      cout << "Bits remaning in the queue "<<bitqueue.size() << endl;
	      byteidx--;
	    }
	  ...
	  if(m_Verbosity > 0)
	    {
	      if(m_Verbosity > 1) 	      
		cout << "Bits remaning in the queue "<<bitqueue.size() << endl;
	      cout << "DECOMPRESSED EVENT STATUS: OK " << endl;
	    }
  ...
  while( adc.size() != nch*seqlen )
      ...
      if(chread == nch)
	{
	  ...
	  if(bitqueue.size() > 0) // should be no more than 8
	    {
	      cout << "Bits remaning in the queue "<<bitqueue.size() << endl;
	      cout << "Current position in file "<<fin.tellg();
	      fin.seekg( -1, std::ios::cur );
	      cout << " Prv position in the file "<<fin.tellg() << endl;
	    }
...
//
// fast low-level function for Huffman decompression
// parameters related to docompression are mostly hardcoded
void dlardaq::DecompressEventDataFast( const void *in, size_t nbytes, size_t seqlen,
				       std::vector<adc16_t> &adc )
{
  ...
  while(buf != stop)
	...
	{
	  cout << "Problem with decompression loop" << endl;
	  break;
	}
      ...
      if( !rawadc && newseq )
	{
	  cout << "New sequence should begin with uncompressed value" << endl;
	  return;
	}
      ...
      if(bitbytep == 0)  // move to next byte
	{
	  ...
	  if( buf == stop ) // finished
	    {
	      cout << "Problem with decoding encountered. Should have more bytes left" << endl;
	      break;
	    }
	}
      ...
      else // otherwise process compressed bits
	{
	  if( adc.empty() ) 
	    {
	      cout << "Fatal decoding error: no previous ADC value found." << endl;
	      return;
	    }
	  ...
	  while(bitsread < bitsdata)
	      ...
	      if( ( (*buf) >> (bitbytep-1) ) & 1 )
		{
		  ...
		  switch (nzeros)
		    ...
		    case 0:
		      if( lastdelta == -999 ) 
			{
			  cout << "Fatal decoding error: no previous delta found." << endl;
			  // cout << bitsread<<" "<<adc.size()<<" "<<adc[adc.size()-3]<<" "
			  // 	  <<adc[adc.size()-1]<<" "<<buf - start<<" "
			  // 	  <<bitset<8>(*(buf-5)) <<" "
			  // 	  <<bitset<8>(*(buf-4)) <<" "
			  // 	  <<bitset<8>(*(buf-3)) <<" "
			  // 	  <<bitset<8>(*(buf-2)) <<" "
		  ...
		  for(int i=0;i<addnew;i++ )
		    {
		      adc16_t val = adc.back() + lastdelta;
		      adc.push_back( val );
		    }
		...
		{ buf++; bitbytep = 8; }
	      if( buf == stop ) break; // finished
	      if( newseq ) break; // finished new sequance
	    } // end while
