...
void PrintHelp(){
  ...
  cout << "  -p indicates that the run is a pedestal run." << endl;
  ...
  cout << "Note that in order not to take too much space in one go, only 10 events can be processed at the same time. Can be ignored with -a" << endl;
  return;
}
...
int main(int argc, char **argv){
  ...
  int last_event = -1;
  bool pedestal = false;
  bool break_on_global_counter = true;
  char cc;

  while((cc = getopt(argc, argv, "r:s:f:l:hpa")) != -1){  
    switch(cc){
      case 'r': 
        run =  atoi(optarg);
        break;  
  ...
  if(pedestal){
    rawdata_folder = "/eos/experiment/wa105/data/311/calibrations/" + to_string(run) + "/";
  }
  ...
  if(subrun < 0 and first_event < 0 and last_event < 0){
    ...
    if(pedestal){
      file = rawdata_folder + to_string(run) + "-" + to_string(subrun) + ".pul.cal";
    }
    if(!ExistTest(file)){
  ...
  for(auto ssubrun : input_files){
    ...
    for(size_t event = 0; event < NEvent; event++){
      ...
      if(glob_event < first_event or glob_event > last_event){continue;}
    	global_counter++;
    	if(global_counter > 10 and break_on_global_counter){return 0;}
