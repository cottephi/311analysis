...
//main function
void RecoFastAnalysisExample(int FirstRun, int LastRun, int NSubruns, std::string Reco){
  ...
  double ADC2CHARGE = 61.; //to be checked
  double ADC2CHARGEView0 = 55.; //to be checked
  double ADC2CHARGEView1 = 67.; //to be checked
  ...
  TH1F *hdQds = new TH1F("hdQds", "Good tracks: dQ/ds for all hits",100,0,50);
  TH1F *hdQdsView0 = new TH1F("hdQdsView0", "Good tracks: dQ/ds for all hits in view 0",100,0,50);
  TH1F *hdQdsView1 = new TH1F("hdQdsView1", "Good tracks: dQ/ds for all hits in view 1",100,0,50);
  ...
  for(int run=FirstRun; run<=LastRun; run++) //Run loop
  {
    ...
    for(int subrun=0; subrun<NSubruns; subrun++) //Subrun loop
    {
      ...
      stringsubrun << subrun;
      std::string StringRootFile( "/eos/experiment/wa105/offline/LArSoft/Data/Reco/" + Reco + "/ROOT/recofast/" + stringrun.str() + "/"  + stringrun.str() + "-" +  stringsubrun.str()  + "-RecoFast-Parser.root");
      std::cout << std::endl;
      ...
      for(int i=0; i<NEntries; i++) //Event loop
      {
        ...
        for(int j=0; j<tNumberOfTracks; j++) //Track loop
        {
          ...
          if(tEventNumberInRun == 554 and j == 0){
            cout << tTrack_StartPoint_X[j] << " " << tTrack_EndPoint_X[j] << " " << TrackXUpperCut << " " << TrackXLowerCut << endl;
            cout << tTrack_StartPoint_Z[j] << " " << tTrack_EndPoint_Z[j] << " " << TrackZUpperCut << " " << TrackZLowerCut << endl;
            cout << tNumberOfTracks << endl;
          }
//          Track selection. Selected tracks = good tracks.
          if( std::max(tTrack_StartPoint_X[j],tTrack_EndPoint_X[j]) > TrackXUpperCut && std::min(tTrack_StartPoint_X[j],tTrack_EndPoint_X[j]) < TrackXLowerCut &&
            ...
            trk++;
            TH1F onehdQdsView0("", "",100,0,50);
            TH1F onehdQdsView1("",  string("Event " + to_string(tEventNumberInRun) + " track " + to_string(j)).data(),100,0,50);
...
//                if(chien < 100){
//                  chien++;
//                  cout << b+l << " " << tTrack_Hit_ChargeSummedADC[l]/(tTrack_Hit_ds_LocalTrackDirection[l]*ADC2CHARGEView0) << endl;
//                }
                tothit0++;
                hdQds->Fill(tTrack_Hit_ChargeSummedADC[l]/(tTrack_Hit_ds_LocalTrackDirection[l]*ADC2CHARGEView0));
                hGooddQdsVsDrift_LocalTrackDirection->Fill(-1*(tTrack_Hit_X[l]-50)/driftvelocity, tTrack_Hit_ChargeSummedADC[l]/(tTrack_Hit_ds_LocalTrackDirection[l]*ADC2CHARGEView0));
                hGooddQdsVsDrift_3DPosition->Fill(-1*(tTrack_Hit_X[l]-50)/driftvelocity, tTrack_Hit_ChargeSummedADC[l]/(tTrack_Hit_ds_LocalTrackDirection[l]*ADC2CHARGEView0));
                hdQdsView0->Fill( tTrack_Hit_ChargeSummedADC[l]/(tTrack_Hit_ds_LocalTrackDirection[l]*ADC2CHARGEView0));
                if(trk < 100){
                  onehdQdsView0.Fill( tTrack_Hit_ChargeSummedADC[l]/(tTrack_Hit_ds_LocalTrackDirection[l]*ADC2CHARGEView0));
                }
                hGooddQdsVsDriftView0->Fill(-1*(tTrack_Hit_X[l]-50)/driftvelocity, tTrack_Hit_ChargeSummedADC[l]/(tTrack_Hit_ds_LocalTrackDirection[l]*ADC2CHARGEView0));
              }
              if(tTrack_Hit_View[l] == 1){ //view 1
                tothit1++;
                HitCountView1b++;
                hdQds->Fill(tTrack_Hit_ChargeSummedADC[l]/(tTrack_Hit_ds_LocalTrackDirection[l]*ADC2CHARGEView1));
                hGooddQdsVsDrift_LocalTrackDirection->Fill(-1*(tTrack_Hit_X[l]-50)/driftvelocity, tTrack_Hit_ChargeSummedADC[l]/(tTrack_Hit_ds_LocalTrackDirection[l]*ADC2CHARGEView1));
                hGooddQdsVsDrift_3DPosition->Fill(-1*(tTrack_Hit_X[l]-50)/driftvelocity, tTrack_Hit_ChargeSummedADC[l]/(tTrack_Hit_ds_LocalTrackDirection[l]*ADC2CHARGEView1));
                hdQdsView1->Fill(tTrack_Hit_ChargeSummedADC[l]/(tTrack_Hit_ds_LocalTrackDirection[l]*ADC2CHARGEView1));
                if(trk < 100){
                  onehdQdsView1.Fill( tTrack_Hit_ChargeSummedADC[l]/(tTrack_Hit_ds_LocalTrackDirection[l]*ADC2CHARGEView0));
                }
                hGooddQdsVsDriftView1->Fill(-1*(tTrack_Hit_X[l]-50)/driftvelocity, tTrack_Hit_ChargeSummedADC[l]/(tTrack_Hit_ds_LocalTrackDirection[l]*ADC2CHARGEView1));
              }
            } //Hit loop (for this track)
            if(trk < 100){
              dQds0s.push_back(onehdQdsView0);
              dQds1s.push_back(onehdQdsView1);
            }
          } //Good tracks
  ...
  TCanvas mycan("","",1000,618);
  for(int i = 0; i < dQds0s.size(); i++){
    dQds1s[i].SetLineColor(kRed);
    dQds1s[i].Draw();
    dQds0s[i].Draw("SAME");
    if(i == 0){
      mycan.Print("./christoph.pdf(","pdf");
    }
    else{
      mycan.Print("./christoph.pdf","pdf");
    }
  ...
  std::string StringRootFileHistograms( "RecoFastAnalysisExampleHistograms_" + Reco + ".root");
  const char* CharRootFileHistograms = StringRootFileHistograms.c_str();

  
  hdQdsView1->SetLineColor(kRed);
  hdQdsView1->Draw();
  hdQdsView0->Draw("SAME");
  mycan.Print("./christoph.pdf)","pdf");
  ...
  hdQdsView0->Write();
  hdQdsView1->Write();
  cout << "Total hits: " << tothit0 << " " << tothit1 << endl;
