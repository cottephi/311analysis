...
void MPV_vs_stuff(int run = 840, string cut_type = "before_cuts", string method = "ds", string version = "June", string m_dQ = "sum", string m_ds = "3D" ){
  method_ds = m_ds;
  method_dQ = m_dQ;
  ...
  #if verbose
  cout << "Doing MPV vs " << method << " for run " << run << endl;
  #endif
  ...
  string outpath = MPV_vs_stuff_Output;
  ...
  if(method == "ds"){
    stuff_min = pitch;
    dstuff = pitch/4.;
    stuff_max = 3.; //max of 10.36 with cuts of 10deg on theta and phi
    name = "ds";
    outpath = outpath + name + ".root";
    ...
    nstuff = 100;
    stuff_max = TMath::Pi();
    dstuff = (stuff_max-stuff_min)/nstuff;
    name = "theta";
    outpath = outpath + name + ".root";
  }
  else if(method == "phi"){
  ...
  for(int i = 0; i < nstuff; i++){
    ...
    dqds_per_stuff.push_back({TH1D(string("dqds_view_0_at_"+name+"_"+str_stuff).data(),string("dqds_view_0_at_"+name+"_"+str_stuff).data(),100,0,50), TH1D(string("dqds_view_1_at_"+name+"_"+str_stuff).data(),string("dqds_view_1_at_"+name+"_"+str_stuff).data(),100,0,50)});
    
    for( auto lem : lems ){
      dqds_per_stuff_ByLEMs[lem].push_back({TH1D(string("dqds_view_0_LEM_"+to_string(lem)+"_at_"+name+"_"+str_stuff).data(),string("dqds_view_0_LEM_"+to_string(lem)+"_at_"+name+"_"+str_stuff).data(),100,0,50), TH1D(string("dqds_view_1_LEM_"+to_string(lem)+"_at_"+name+"_"+str_stuff).data(),string("dqds_view_1_LEM_"+to_string(lem)+"_at_"+name+"_"+str_stuff).data(),100,0,50)});
    }
  }
  for( auto t : tracks){
    for( auto h : t.hits_trk ){
      ...
      int istuff = -1;
      if(method == "ds" && !MC){istuff = (int)((ds-stuff_min)/dstuff);}
      else if(method == "ds" && MC){istuff = (int)((h.dx_MC-stuff_min)/dstuff);}
      else if(method == "theta"){istuff = (int)((t.theta-stuff_min)/dstuff);}
      else if(method == "phi"){istuff = (int)((t.phi-stuff_min)/dstuff);}
      else if(method == "GoF"){istuff = (int)((h.GoF-stuff_min)/dstuff);}
  ...
  mpv_vs_stuff[0].SetName(string("mpv_vs_"+name+"view_0").data());
  ...
  mpv_vs_stuff[1].SetName(string("mpv_vs_"+name+"view_1").data());
  ...
  mpv_vs_stuff[2].SetName(string("summed_views_mpv_vs_"+name).data());
  ...
  for( auto lem : lems ){
    mpv_vs_stuff_ByLEMs[lem][0].SetName(string("mpv_vs_"+name+"_lem_"+to_string(lem)+"view_0").data());
    ...
    mpv_vs_stuff_ByLEMs[lem][1].SetName(string("mpv_vs_"+name+"_lem_"+to_string(lem)+"view_1").data());
    ...
    mpv_vs_stuff_ByLEMs[lem][2].SetName(string("summed_views_mpv_vs_"+name+"_lem_"+to_string(lem)).data());
