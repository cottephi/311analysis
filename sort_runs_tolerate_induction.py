...
def check_diff(df_run_ref, df_run_to_compare):#Count how many fields are different between the two input DataFrames. A difference inferior to 5% in Drift will be considered as an equality
  ...
  for field in df_run_ref:
    if (float(df_run_ref[field]) != float(df_run_to_compare[field]) and field != "Drift" and field != "Induction") or ( (abs(float(df_run_ref[field]) - float(df_run_to_compare[field]))/float(df_run_ref[field]) > drift_limit or abs(float(df_run_ref[field]) - float(df_run_to_compare[field]))/float(df_run_to_compare[field]) > drift_limit) and field == "Drift"):
      diff = diff + 1
      field_to_scan = field
...
if output_format == "c++":
  ...
  for scan_field in runs_sorted_by_scan:
    ...
    for runs in runs_sorted_by_scan[scan_field]:
      ...
      [ print("   //",field,":",float(runs[0][field])/1000.,' ', end='') for field in fields_to_print if field != "Induction"]
      outstring = '\n    run_list = {'
      for run in runs:
