...
void
evd_311(std::string const& filename, size_t ievcount, std::string const& inputtag="caldata", bool autoped=false, int minval=-10, int maxval=30)
{
  ...
  for (gallery::Event ev(filenames); !ev.atEnd(); ev.next()) {
    if (evcounter == ievcount)
      {
	auto const& wires = *ev.getValidHandle<vector<recob::Wire>>(wire_tag);
	if (!wires.empty())
	  {
	    size_t nwires = wires.size();
	    size_t nchans=0;
	    for (size_t i=0; i<nwires; ++i)
	      {
		size_t ic = wires[i].Channel();
		if (nchans<ic) nchans=ic;
	      }
	    nchans++;  // set plots to go from channel 0 to the maximum channel we find.

	    size_t nticks = wires[0].Signal().size();  // assume uncompressed, all channels have the same
                                                    // number of samples.
		...
		if (autoped)
		  {
		    ...
		    for (size_t itick=0;itick<nticks;++itick)
		      {
			csum += wires[ichan].Signal().at(itick);
		      }
		    csum /= ( (double) nticks );
		    iped = (int) csum;
		  }
		for (size_t itick=0;itick<nticks;++itick)
		  {
		    hview0->SetBinContent(ic+1,itick+1,wires[ichan].Signal().at(nticks-itick-1)-iped);
		  }
	      }

	    TH2I *hview1 = (TH2I*) new TH2I("hview1","View 1",960,321-0.5,1280-0.5,nticks,-1667+0.5,0.5);
	    hview1->SetDirectory(0);
	    for (size_t ichan=320;ichan<nwires; ++ichan)
	      {
		size_t ic = wires[ichan].Channel();
		int iped = 0;  // an integer for raw digits
		if (autoped)
		...
		for (size_t itick=0;itick<nticks;++itick)
		  {
		    ...
		    hview1->SetBinContent(ic+1-320,itick+1,wires[ichan].Signal().at(nticks-itick-1)-iped);
		  }
	      }

	    hview0->GetXaxis()->SetLabelSize(0.04);
	    ...
	    hview0->GetYaxis()->SetLabelSize(0.04);
	    hview0->GetYaxis()->SetLabelOffset(0.01);
	    hview0->GetYaxis()->SetTitleSize(0.06);
	    hview0->GetYaxis()->SetTitleOffset(1.40);
